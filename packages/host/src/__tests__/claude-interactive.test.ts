...
describe('Claude Interactive Service', () => {
  ...
  it('should not create duplicate sessions', async () => {
    ...
    await expect(service.createSession('test-session')).rejects.toThrow(
  ...
  it('should reject commands for non-existent session', async () => {
    await expect(
      service.sendCommand('non-existent', 'test command')
...
it('should start claude CLI and print version', async () => {
  // このテストだけはchild_processのモックを解除
  vi.unmock('child_process');
  const { execFile } = await import('child_process');
  await new Promise<void>((resolve, reject) => {
    execFile('claude', ['--version'], (error, stdout, _) => {
      try {
        expect(error).toBeNull();
        expect(stdout).toMatch(/\d+\.\d+\.\d+/); // バージョン番号が出力されること
        resolve();
      } catch (e) {
        reject(e);
      }
